...
public sealed class GetWorkshopsHandler(WorkshopStore store)
    : IRequestHandler<GetWorkshopsQuery, Result<IReadOnlyList<WorkshopSummaryResponse>>>
{
    public Task<Result<IReadOnlyList<WorkshopSummaryResponse>>> Handle(GetWorkshopsQuery request, CancellationToken ct)
    {
        ...
        return Task.FromResult(Result<IReadOnlyList<WorkshopSummaryResponse>>.Success(workshops));
    }
}
