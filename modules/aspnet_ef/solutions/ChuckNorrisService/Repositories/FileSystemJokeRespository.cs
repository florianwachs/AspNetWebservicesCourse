...
public class FileSystemJokeRespository : IJokeRepository
{
    ...
    private void Init()
    {
        ...
        var allCategories = jokeDtos
            ...
            .Select(cat => new JokeCategory() { Id = cat, Name = cat })
            .ToDictionary(k => k.Id);
        ...
        {
            var matchingCategories = jokeDto.Category?.Select(cat => allCategories[cat]).ToList();
            result.Add(
                new Joke
                {
                    Id = jokeDto.Id,
                    Value = jokeDto.Value,
                    Categories = matchingCategories ?? new()
                });
        }
